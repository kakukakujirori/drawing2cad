...
Writes `<stem>_dim.dxf` and `<stem>_dim.png` per input. `--seed` selects the
annotation style (one style per drawing, reproducible), `--dpi` and
`--text-height` override the render resolution and text size.

```python
from dim_drawer import annotate

dxf, png, n_dims, n_views, thick, thin = annotate("part.dxf", "out/", seed=0)
```

## How it works

1. `extract.py` — reads the DXF, expanding blocks and polylines and flattening
   splines; dashed linetypes become hidden lines.
...
4. `dimensions.py` — draws the DIMENSION entities. Each one is rendered, its real
   text box read back from the generated block, and undone if it collides with
   something already placed.

Style is randomised per drawing: arrow blocks, text style, decimal separator and
count, optional `mm` suffix and background fill, ISO 128 line group
(0.25/0.35/0.5/0.7 mm at 2:1 thick-to-thin) and dash length.
