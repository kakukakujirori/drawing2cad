...
def lsd_lines(gray):
    try:
        ...
        if segs is None:
            return []
        return [[float(x1), float(y1), float(x2), float(y2)] for [[x1, y1, x2, y2]] in segs]
...
def hough_lines(gray):
    ...
    lines = cv2.HoughLinesP(edges, 1, np.pi/180, threshold=50, minLineLength=25, maxLineGap=5)
    if lines is None:
        return []
...
def hough_circles(gray):
    ...
    for (rmin, rmax) in [(8, 22), (20, 45), (40, 90), (85, 200)]:   # radius bands
        c = cv2.HoughCircles(blur, cv2.HOUGH_GRADIENT, dp=1.2, minDist=18,
                             param1=120, param2=42, minRadius=rmin, maxRadius=rmax)
        if c is not None:
...
def main():
    ...
    for variant in ("clean", "scan"):
        ...
        rows.append((variant, "LSD(lines)", len(det_lsd["lines"]), s_lsd["lines"], None))
        rows.append((variant, "HoughP(lines)+HoughCircles", len(det_hough["lines"]),
                     s_hough["lines"], s_hough["circles"]))
        json.dump({"lsd": s_lsd, "hough": s_hough,
                   "n_lsd_lines": len(det_lsd["lines"]),
                   "n_hough_lines": len(det_hough["lines"]),
                   "n_hough_circles": len(det_hough["circles"])},
                  open(os.path.join(a.dir, f"opencv_{variant}.json"), "w"), indent=2)
    print(f"\n{'variant':6} {'method':28} {'#det':>5}  line R(vis/hid/all) "
          f"line P   circle R/P")
    for variant, name, ndet, ln, ci in rows:
        lr = f"{ln['visible']}/{ln['hidden']}/{ln['all']}"
        cstr = f"{ci['recall']}/{ci['precision']} ({ci['tp']}/{ci['gt']})" if ci else "-"
        print(f"{variant:6} {name:28} {ndet:>5}  {lr:18} {str(ln['precision']):7} {cstr}")
