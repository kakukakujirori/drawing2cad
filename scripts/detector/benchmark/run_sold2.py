...
def main():
    ...
    for variant in ("clean", "scan"):
        ...
        H, W = img.shape; s = a.long / max(H, W)
        im = cv2.resize(img, (int(W * s), int(H * s)))
        t = torch.from_numpy(im).float()[None, None] / 255.
        t0 = time.time()
        with torch.no_grad():
            seg = sold2(t)["line_segments"][0].cpu().numpy()   # [N,2,2], (row,col)=(y,x) per endpoint
        lines = [[float(p0[1]/s), float(p0[0]/s), float(p1[1]/s), float(p1[0]/s)] for p0, p1 in seg]
        sc = score(graph, {"lines": lines, "circles": []})
        rows.append((variant, len(lines), sc["lines"], time.time() - t0))
        json.dump({"n_lines": len(lines), "lines": sc["lines"]},
                  open(os.path.join(a.dir, f"sold2_{variant}.json"), "w"), indent=2)
