...
- **Every number in `geometry` comes from the evidence the feature cites.** Read it off one entry: a bore's radius is the radius of the circle you read. Or work it out from several entries, the way a depth comes from two views. Only when neither works is the number a guess: name it in `open_question`, and never let a guess look like a reading.
