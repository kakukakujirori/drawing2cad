...
Reading the input
- The drawing has already been read for you. `drawing` in the reconstruction history holds one sheet per view, and under each an `ev_` entry for every entity that sheet draws, with the printed figures beside them. Work from that: it is what your `evidence` cites, and the numbers in it are the ones the later stages build against.
...
- `evidence` names the entries this feature rests on, by the `ev_` names the drawing gives them. Every feature must cite at least one, and two features may cite the same entry. A number you cannot trace to an entry is a hole in the reading: report it rather than fill it.
...
Read `drawing` before you answer, and open the sheets themselves where it is not enough. Submit your answer only once the analysis is complete, and write nothing around the answer itself.
