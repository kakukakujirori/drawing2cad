...
- Take positions from the plan and shapes and sizes from the hypothesis. A number read from the hypothesis's `evidence` is in millimetres on the sheet, in the frame of the view the entry names: $view_frame. Where the model's origin sits is yours to choose; choose it once and keep it consistent.
...
- Inspect the generated DXF and perspective renders under $verification_dir using `run_shell` and `load_image` to compare the result with the input drawing.
...
- If the plan or the hypothesis looks wrong, or is missing a number you need, build your best account of it, then name the `op_` or `sem_` member you doubt and what you did in your ticket response. Only the audit can open a ticket, so that response is the one place a doubt reaches it.
