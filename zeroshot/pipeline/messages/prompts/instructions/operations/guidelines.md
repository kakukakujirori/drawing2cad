...
- Its `evidence` holds numbers read straight off the drawing, in millimetres on the sheet, in the frame of the view each entry names: $view_frame.
...
- **Do not copy a number out of the hypothesis.** Cite a 3D claim as `sem_<feature>.geo_<claim>.<parameter>` and something read off the drawing as `ev_<evidence>.<parameter>` -- for example `sem_main_bore.geo_cylinder.radius` or `ev_front_left_edge.start` -- and it is filled in for you before the coder reads it. The `sem_`, `geo_`, and `ev_` names and the parameter name are printed in the hypothesis. Write out only the numbers the hypothesis does not hold: a depth you chose, an offset you worked out, a clearance.
...
Plain numbers in them are allowed if they are not present in the semantic hypothesis. Anything it holds should be cited instead, like `sem_main_bore.geo_cylinder.radius` or `ev_front_circle.center`.
