...
Convert the current semantic hypothesis and operation plan supplied in each instruction into a complete, executable CadQuery Python script written to:
...
- Follow the current operation plan as the construction sequence, while checking its geometry against the current semantic hypothesis and the input drawing.
