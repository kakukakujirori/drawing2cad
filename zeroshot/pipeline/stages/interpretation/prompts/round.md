...
After the first pass across the views, save a provisional artifact by turn 5, with unresolved issues in questions. Use subsequent image inspections to resolve those issues and complete the major geometry, then resubmit. Reserve turns to read the automatic validation and calibration feedback, correct errors, and submit one InterpretationSubmission after the current artifact validates. A valid file confirms the contract and scale checks; it does not confirm the 3D interpretation.
