...
Inside a snapshot, `.interpretation.views[]` holds registered input files and cropped views with `view_` names; each view's `.dimensions[]` holds `dim_` readings. `.interpretation.hypotheses[].candidates[]` holds `sem_` readings; the most confident per hypothesis is adopted. Operations are in `.operations.proposal[]`; code is the `.program_source` string.
