...
Read relevant `stage_reports` too; missing reports in old snapshots mean no report was recorded. For reasoning-stage `TicketAnswers`, each ticket's summary states its outcome, including upstream blockers or provisional interpretations needed to explain it. Use `remark` for additional concerns outside those answers, empty if none. Avoid repeating details: a shared explanation may appear once in remark, but each summary must state its own outcome and impact.
`dimension_checks` is coding's per-dimension report; other reasoning stages leave it null.
