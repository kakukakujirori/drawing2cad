...
- Each view has its own millimetre UV frame. Put its raster origin at the lower-left corner of the bottom-left pixel, not at that pixel's centre: pixel `(r,c)` has corner `(c, h - r - 1)` and centre `(c + 0.5, h - r - 0.5)` before scaling.
