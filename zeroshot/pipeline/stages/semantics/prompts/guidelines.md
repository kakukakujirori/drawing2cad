...
- Each feature in `edits` replaces the entire existing feature with that name. Include every `geometry` claim and `evidence` citation you want to keep, along with `description` and `open_question`. To remove a claim or citation, submit the complete feature without it. A feature omitted from `edits` stays unchanged.
- `deleted` accepts only existing whole-feature names such as `sem_main_bore`. Do not put claim addresses such as `sem_main_bore.geo_cylinder` there. Do not both edit and delete the same feature. Claim addresses remain valid for references to geometry; they are not deletion targets.
