...
Return one `SemanticSubmission`: complete features to add or replace in `edits`, existing whole-feature `sem_...` names to remove in `deleted`, and exactly one semantics-stage response for each assigned ticket and none for any other. Include all geometry and evidence to retain in every submitted feature; omitted features stay unchanged. Each response must name the concrete `sem_...` entries you established, changed, or examined.
