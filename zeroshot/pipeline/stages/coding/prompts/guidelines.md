...
- In `dimension_checks`, cover every supplied dim ID once. For each, identify its realization in the final geometry and the evidence that it holds, or explicitly state why it remains unestablished or unverified. When measured, name the geometry measured and the result; assigning a number to a variable alone does not prove it survived later operations. Include unreadable (null) dimensions and separate IDs with equal values; the same physical realization may support both. Use `{}` if no dimensions exist. Do not repeat this table in ticket summaries or remark, or narrate routine unit/radius conversions.
- If the plan or the interpretation looks wrong, or is missing a number you need, build your best reading, then report the `op_` or `sem_` member you doubt and what you did, following the summary/remark rule. Only the audit can open a ticket.
