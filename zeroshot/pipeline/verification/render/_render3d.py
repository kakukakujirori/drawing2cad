...
def _polylines_from_compound(compound, deflection):
    ...
    while exp.More():
        ...
        except Exception:  # noqa: BLE001, S112 - skip an unreadable edge
            continue
        ...
        try:
            ...
            if len(faces) == 2:
                ...
                if _smooth_same_curvature(edge, faces[0], faces[1], diag):
                    continue
        except Exception:  # noqa: BLE001, S110 - malformed topology: keep the ink
            pass
        edges.append(edge)
    return edges
...
def _mesh_hlr_project(
    ...
    for edge in _drawable_edges(shape, tol=1e-4 * diag, diag=diag):
        ...
        except Exception:  # noqa: BLE001, S112 - skip broken edge geometry
            continue
...
def _composite_translucent(
    ...
    try:
        ...
        with Image.open(line_png_path) as line_image:
            lines = line_image.convert("RGBA")
    finally:
...
def generate_render3d(
    ...
    """Render all styles and return successful style names and diagnostics."""
    step_path = Path(step_path)
    cfg = cfg or Render3dConfig()
    info: dict = {"rendered": [], "errors": {}}
    ...
    try:
        visible, hidden, tf = _render_hlg(
            ...
            camera,
            cfg,
            Path(paths.hlg_perspective),
            tf=sphere_tf,
            verts=verts,
            tris=tris,
        )
        info["rendered"].append("hlg_perspective")
    except Exception as exc:  # noqa: BLE001
        info["errors"]["hlg_perspective"] = f"{type(exc).__name__}: {exc}"
    ...
    if tf is not None and verts is not None:
        try:
            shaded_img = _render_shaded(
                ...
                cfg,
                tf,
                Path(paths.transparent_shaded_edges_perspective),
                verts=verts,
                tris=tris,
            )
            ...
            out = Path(paths.transparent_shaded_edges_perspective)
            out.parent.mkdir(parents=True, exist_ok=True)
            composed.convert("RGB").save(out)
            info["rendered"].append("transparent_shaded_edges_perspective")
        except Exception as exc:  # noqa: BLE001
            info["errors"]["transparent_shaded_edges_perspective"] = (
        ...
        try:
            if shaded_img is not None:
                _composite_translucent(
                    ...
                    tf,
                    cfg,
                    Path(paths.hlg_translucent_faces_perspective),
                )
            else:
                # shaded pass failed: fall back to the plain line art so the
                # style still exists (documented degraded output).
                Path(paths.hlg_translucent_faces_perspective).parent.mkdir(
                    parents=True, exist_ok=True
                )
                svg = _write_hlg_svg(visible, hidden, tf, cfg)
                cairosvg.svg2png(
                    bytestring=svg.encode(),
                    write_to=str(paths.hlg_translucent_faces_perspective),
                    output_width=cfg.canvas_w,
                    output_height=cfg.canvas_h,
                )
            info["rendered"].append("hlg_translucent_faces_perspective")
        except Exception as exc:  # noqa: BLE001
            info["errors"]["hlg_translucent_faces_perspective"] = (
                f"{type(exc).__name__}: {exc}"
            )

    rendered_styles = set(info["rendered"])
    raw_errors = dict(info["errors"])
    fallback = "; ".join(f"{stage}: {message}" for stage, message in raw_errors.items())
    info["errors"] = {
        path_field.name: (
            raw_errors.get(path_field.name)
            or fallback
            or f"{path_field.name} was not generated"
        )
        for path_field in fields(paths)
        if path_field.name not in rendered_styles
    }
    return info
...
if __name__ == "__main__":  # pragma: no cover - manual smoke test
    ...
    rp = Render3dPaths.from_path(out / "render_3d", step.stem)
