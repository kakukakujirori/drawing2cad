...
def op(
    ...
    *,
    ...
    return Operation(
        id=identifier,
        verb=verb,
        detail=detail or f"operation {identifier}",
...
def test_a_feature_no_operation_builds_is_reported() -> None:
    ...
    review = review_plan(plan(op(1, builds=[1]), op(2, builds=[3])), _features(1, 2, 3))
...
def test_a_feature_the_plan_invents_is_reported() -> None:
    review = review_plan(plan(op(1, builds=[9])), _features(1))
...
def test_a_plan_that_accounts_for_every_feature_is_complete() -> None:
    review = review_plan(plan(op(1, builds=[1, 2]), op(2, builds=[2])), _features(1, 2))
...
def test_the_rendering_shows_the_derived_order_and_what_each_step_needs() -> None:
    ...
    rendered = render_plan(
        plan(op(2, needs=[1], builds=[4]), op(1, builds=[1])), _features(1, 4)
    )
    lines = rendered.splitlines()

    assert "op1 extrude (from nothing; builds sem1)" in lines[1]
    assert "op2 extrude (after op1; builds sem4)" in lines[2]
...
def test_an_operation_that_names_no_feature_says_so() -> None:
    ...
    assert "builds nothing named" in render_plan(plan(op(1)), _features(1))
...
def test_the_gap_is_reported_by_naming_the_features_it_left_out() -> None:
    ...
    review = review_plan(plan(op(1, builds=[1])), _features(1, 2, 5))
...
def test_a_complete_plan_is_reported_as_nothing_at_all() -> None:
    ...
    assert render_plan_review(review_plan(plan(op(1, builds=[1])), _features(1))) == ""
...
def test_a_reading_disowns_a_pair_that_has_moved_on(changed: str) -> None:
    ...
    if changed == "hypothesis":
        ...
        assert not review.describes(established, plan(op(1, builds=[1])))
...
def test_a_number_the_hypothesis_already_holds_is_refused() -> None:
    ...
    review = review_plan(
        plan(op(1, builds=[7], detail="Sweep a blend of radius 11.31245992416")),
        _torus_feature(),
    )

    assert review.transcribed == {1: ["11.31245992416"]}
    assert not review.sound
...
def test_a_short_number_is_the_planners_own_words() -> None:
    ...
    assert (
        review_plan(plan(op(1, builds=[7], detail="Extrude 25 mm")), held).transcribed
        == {}
    )
...
def test_a_reference_that_stands_for_nothing_is_refused() -> None:
    review = review_plan(
        plan(op(1, builds=[7], detail="Sweep sem7.height along +z")), _torus_feature()
    )

    assert review.unresolved == {1: ["sem7.height"]}
...
def test_a_plan_full_of_copied_numbers_is_still_told_in_a_sentence() -> None:
    ...
    told = render_plan_review(review_plan(plan(op(1, builds=[7], detail=copied)), held))
