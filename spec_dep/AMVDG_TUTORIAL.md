...
> **注記 (v0.3)**: 以下のコードスニペットは `example_flange_v0.3.json`（`spec/flange.step` をレンダラに通した実出力：プレート⌀120・ハブ⌀76・ボア⌀40・ボルト穴⌀11×6）から実際に抜き出したものです。ただし `features[].members` のように長い配列は `…` で省略しています（実ファイルは全メンバを持ちます）。v0.3 で追加された `prov.topo_origins`（2Dプリミティブの3D由来）・中心線プリミティブ・円弧の `start_angle`/`end_angle` は [`spec/AMVDG_v0.3.md`](AMVDG_v0.3.md) の **§9「v0.3 delta」** を参照してください。
>
...
```
ここから、この図面が **A3サイズ**、**第三角法**、図面スケール **1:2** で描かれており、画像化された際の解像度が **1800x1273 px**（1mmあたり約4.28ピクセル）であることが分かります。この `px_per_mm` が、後述の2D座標を実寸にスケールする際の基準になります。`profile` はこのグラフの完成度（`vectorized` = 座標付き）を示します（詳細は [`spec/AMVDG_v0.3.md`](AMVDG_v0.3.md) §3 の検証プロファイル。なお現状レンダラが出すのは `vectorized` のみで、`derived`/`gt_executable` はスキーマ予約・未生成です）。
