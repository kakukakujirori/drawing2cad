...
def check_refs(g):
    ...
    for v in g["views"]:
        for p in v["primitives"]:
            prim_ids.add(p["id"])
            prim_view[p["id"]] = v["name"]
    feat_ids = {f["feature_id"] for f in g["features"]}
    # annotation refs -> primitive ids
    for a in g["annotations"]:
        for r in a.get("refs", []):
            if r not in prim_ids:
                errs.append(f"ann {a['id']} ref '{r}' not a primitive")
    # feature members -> primitives ; feature.parent -> feature
    member_prims = set()
    for f in g["features"]:
        for m in f["members"]:
            member_prims.add(m["primitive_id"])
            if m["primitive_id"] not in prim_ids:
                errs.append(
                    f"feat {f['feature_id']} member '{m['primitive_id']}' not a primitive"
                )
    ...
    for v in g["views"]:
        for p in v["primitives"]:
            ...
            if fid and fid not in feat_ids:
                errs.append(f"prim {p['id']} feature_id '{fid}' not a feature")
    # parent cycle
    ...
    for fid in par:
        ...
        while cur:
            if cur in seen:
                errs.append(f"parent cycle at {fid}")
                break
            seen.add(cur)
            cur = par.get(cur)
    return errs
...
def check_census(g):
    ...
    for vname, counts in by.items():
        ...
        if not v:
            errs.append(f"census view {vname} missing")
            continue
        ...
        for k, decl in counts.items():
            if k in actual and actual[k] != decl:
                errs.append(
                    f"census {vname}.{k}: declared {decl} != emitted {actual[k]}"
                )
    return errs
...
def check_dof(g):
    ...
    for f in g["features"]:
        ...
        for p in d.get("determined_params", []):
            determined_set.add(f"{f['feature_id']}.{p}")
    missing = req - det - detg
    D = g["dof"]
    for k, calc in (
        ("required", req),
        ("determined", det),
        ("determined_by_geometry", detg),
        ("missing", missing),
    ):
        if D.get(k) != calc:
            errs.append(f"dof.{k}: stated {D.get(k)} != recomputed {calc}")
    sd = D.get("self_declared") or {}
    for k in ("required", "determined", "missing"):
        if sd.get(k) != D.get(k):
            errs.append(f"self_declared.{k} {sd.get(k)} != dof.{k} {D.get(k)}")
    # abstain disjoint
    for u in D.get("undetermined", []):
        if u in determined_set:
            errs.append(f"undetermined '{u}' also marked determined")
    # coverage
    cov = round(det / req, 4) if req else 0
    if abs((D.get("coverage") or 0) - cov) > 1e-3:
        errs.append(f"coverage {D.get('coverage')} != {cov}")
    return errs
...
def _has_coords(p):
    t = p["type"]
    if t == "line":
        return p.get("p1") and p.get("p2")
    if t in ("circle", "arc"):
        return p.get("center") and p.get("r_px") is not None
    if t == "polyline":
        return bool(p.get("pts"))
    return True
...
def check_profile(g):
    ...
    if need_coords:
        ...
        for v in g["views"]:
            ...
            if missing:
                errs.append(
                    f"[{prof}] view {v['name']} frame missing: {', '.join(missing)}"
                )
        # linear dims without horizontal/vertical subtype have no defined measurement span
        for a in g["annotations"]:
            if a.get("kind") == "linear" and a.get("subtype") not in (
                "horizontal",
                "vertical",
            ):
                errs.append(
                    f"[{prof}] linear ann {a['id']} subtype "
        ...
        for v in g["views"]:
            for p in v["primitives"]:
                if p.get("state") == "known" and not _has_coords(p):
                    errs.append(
                        f"[{prof}] prim {p['id']} state=known but missing coords"
                    )
                if p.get("coords_source", "unknown") == "unknown":
                    errs.append(f"[{prof}] prim {p['id']} coords_source=unknown")
    if need_exec:
        ann_known = {
            a["id"] for a in g["annotations"] if a.get("value_state") == "known"
        }
        for f in g["features"]:
            if not f.get("prov"):
                errs.append(f"[gt] feat {f['feature_id']} missing prov")
            d = f.get("dof") or {}
            if d.get("status") == "determined":
                b = f.get("build")
                if not b or not b.get("op") or not b.get("datum_plane"):
                    errs.append(
                        f"[gt] determined feat {f['feature_id']} build incomplete (op/datum_plane)"
                    )
                    continue
                ...
                for rp in d.get("required_params", []):
                    ...
                    if not ptr:
                        errs.append(
                            f"[gt] feat {f['feature_id']} required '{rp}' has null driven_by"
                        )
                    elif ptr not in ann_known:
                        errs.append(
                            f"[gt] feat {f['feature_id']} '{rp}' -> {ptr} not a value_state=known annotation"
                        )
    return errs
...
def fmt(
    x,
):  # canonical number token so lower(lift(s))==s (int 0 and float 0.0 both -> "0")
    f = float(x)
...
def lower(g):
    ...
    for v in g["views"]:
        ar = v.get("align_to")
        al = f"{ar['view']}:{ar['shared_axis']}" if ar else "-"
        L.append(
            f"V|{v['name']}|{v['view_type']}|{','.join(fmt(x) for x in v['projection_dir'])}|{al}"
        )
    for a in g["annotations"]:
        L.append(
            f"A|{a['id']}|{a['kind']}|{a.get('param_role', 'unknown')}|{a.get('feature_id') or '-'}|"
            f"{'-' if a.get('value') is None else fmt(a['value'])}|{a['value_state']}|{'.'.join(a.get('refs', [])) or '-'}"
        )
    for f in g["features"]:
        mem = ".".join(
            f"{m['view']}:{m['primitive_id']}:{m['projection_role']}"
            for m in f["members"]
        )
        L.append(
            f"F|{f['feature_id']}|{f['kind']}|{f.get('axis', 'none')}|{f.get('extrude_dir') or '-'}|"
...
def lift(text):
    ...
    for line in text.split("\n"):
        ...
        if t[0] == "V":
            ...
            if t[4] != "-":
                v_, ax = t[4].split(":")
                ar = {"view": v_, "shared_axis": ax}
            g["views"].append(
                {
                    "name": t[1],
                    "view_type": t[2],
                    "projection_dir": [float(x) for x in t[3].split(",")],
                    "align_to": ar,
                }
            )
        elif t[0] == "A":
            g["annotations"].append(
                {
                    ...
                    "value": None if t[5] == "-" else float(t[5]),
                    "value_state": t[6],
            ...
            if t[7]:
                for ms in t[7].split("."):
                    vv, pp, rr = ms.split(":")
                    mem.append({"view": vv, "primitive_id": pp, "projection_role": rr})
            g["features"].append(
                {
                    "feature_id": t[1],
                    "kind": t[2],
                    "axis": t[3],
                    "extrude_dir": None if t[4] == "-" else t[4],
                    "status": t[5],
                    "parent": None if t[6] == "-" else t[6],
                    "members": mem,
                }
            )
    return g
...
def main():
    ...
    schema = load(
        sys.argv[2]
        if len(sys.argv) > 2
        else os.path.join(here, "AMVDG_v0.3.schema.json")
    )
    res = {}
    try:
