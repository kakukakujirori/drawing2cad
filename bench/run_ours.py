...
def main() -> int:
    ...
    parser.add_argument("--ckpt", type=Path, required=True, help="LoRA adapter dir")
    parser.add_argument(
        "--run-name", type=str, required=True, help="results/<run_name>/ subdir"
    )
    parser.add_argument(
        "--quant",
        type=int,
    ...
    preds_dir = (
        (run_dir / "_preds")
        if args.keep_preds
        else Path(tempfile.mkdtemp(prefix="bench_preds_"))
    )
    ...
    infer_hist = (
        (run_meta.get("infer_summary") or {}).get("summary", {}).get("error_hist", {})
    )
    if infer_hist:
    ...
    if n_step == 0 and ids:
        ...
        print(
            f"[run_ours] 0/{len(ids)} fixtures produced a STEP -> hard failure.",
            file=sys.stderr,
        )
        print(
            f"[run_ours] real error+trace: infer.py's stderr above; generated code: "
