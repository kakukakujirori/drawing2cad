...
def main() -> int:
    ...
    ap.add_argument(
        "--all", action="store_true", help="run every fixture in data/inputs"
    )
    ap.add_argument(
        "--backend",
        default="cadquery",
    ...
    ap.add_argument(
        "--max-iter", type=int, default=12, help="agent turn cap (default 12)"
    )
    ap.add_argument(
        "--max-duration",
        type=float,
    ...
    if not args.no_xvfb:
        ...
        if xvfb is None:
            print(
                "[warn] xvfb-run not found; rendering feedback may segfault.",
                file=sys.stderr,
            )
        else:
            cmd = [xvfb, "-a"] + cgb
