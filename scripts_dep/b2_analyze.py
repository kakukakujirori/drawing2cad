...
print(
    f"=== (a) B2-cadrille baseline on {N} generation fixtures (feed={os.path.basename(d)}) ==="
)
...
if extents:
    ...
    print(
        f"    min/med/max = {min(extents):.0f}/{st.median(extents):.0f}/{max(extents):.0f}  "
        f"mean={st.mean(extents):.1f}  CoV={cov:.3f}"
    )
    print(
        f"    within [180,220] (~200 norm box): {near200}/{len(extents)} ({near200 / len(extents):.0%})"
    )
if mags:
    near200m = sum(180 <= m <= 220 for m in mags)
    print(
        f"  extrude/cylinder magnitudes over {len(mags)} ops: "
        f"med={st.median(mags):.0f}  ==200: {mags.count(200.0)}  in[180,220]: {near200m} ({near200m / len(mags):.0%})"
    )
print(
    "  -> interpretation: real mechanical parts span widely (tens..hundreds mm); cadrille"
)
