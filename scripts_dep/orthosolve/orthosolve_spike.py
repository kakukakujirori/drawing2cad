...
def anns(g):
    return g.get("annotations") or g.get(
        "dimensions", []
    )  # v0.2 'annotations' / v0 'dimensions'


def dkind(d):
    return d.get("kind", d.get("type"))  # v0.2 'kind' / v0 'type'
...
print(
    f"{'dim':5} {'mm':>5} {'r_px from DIM':>14} {'GT r_px':>9} {'scan r_px':>10} {'dim_err':>8} {'scan_err':>9}"
)
...
for did, d in gtdim.items():
    ...
    print(
        f"{did:5} {d['value']:>5.0f} {r_dim:>14.2f} {r_gt:>9.2f} {r_scan:>10.2f} {dim_err:>8.2f} {scan_err:>9.2f}"
    )
print(
    "-> DIM-driven radius error ~0 (algebra); PIXEL(scan) radius error large. Scan-invariant by construction."
)
...
def resid(v):
    ...
    x0, y0, x1, y1, x2, y2, x3, y3, cx, cy, r = v
    e = []
    ...
    e += [
        cx - (x0 + x1) / 2,
        cy - (y0 + y3) / 2,
    ]  # concentric: bore center = plate center   -> 2
    return np.array(e, float)  # 11 residuals for 11 vars -> determined


def jac(v, eps=1e-6):
    base = resid(v)
    J = np.zeros((len(base), len(v)))
    for j in range(len(v)):
        vv = v.copy()
        vv[j] += eps
        J[:, j] = (resid(vv) - base) / eps
    return J


rng = np.random.default_rng(0)
seed = true + rng.normal(
    0, 8.0, true.shape
)  # 8mm-scale corruption ~ scan pixel error magnitude
v = seed.copy()
for _ in range(50):
    r = resid(v)
    J = jac(v)
    dv, *_ = np.linalg.lstsq(J, -r, rcond=None)
    v = v + dv
    if np.linalg.norm(dv) < 1e-9:
        break
seed_err = np.abs(seed - true)
solved_err = np.abs(v - true)
print(
    f"{'var':4} {'true':>8} {'noisy seed':>11} {'solved':>9} {'seed_err':>9} {'solved_err':>11}"
)
for i, n in enumerate(names):
    print(
        f"{n:4} {true[i]:>8.2f} {seed[i]:>11.2f} {v[i]:>9.3f} {seed_err[i]:>9.2f} {solved_err[i]:>11.2e}"
    )
print(
    f"-> max seed err {seed_err.max():.2f}mm  ->  max SOLVED err {solved_err.max():.2e}mm  (solver computes exact metric from a wrong seed)"
)

# ============ CHECK 3: REAL/101 — over-determined SCALE recovery from multiple bound diameters ============
print("\n" + "=" * 78)
print(
    "CHECK 3 — HARD (real/101): recover px_per_mm from multiple bound ⌀-dims; consistency = self-check"
)
print("=" * 78)
    ...
    if dkind(d) == "diameter" and d.get("refs"):
        for rid in d["refs"]:
            if rid in rc:
                ...
                print(
                    f"{d['id']:5} {d['value']:>6.0f} {rid:>14} {rpx:>9.1f} {ppm_i:>13.3f}"
                )
import statistics as st

if ppm_votes:
    med = st.median(ppm_votes)
    sd = st.pstdev(ppm_votes)
    cv = sd / med if med else float("nan")
    print(
        f"-> {len(ppm_votes)} scale votes  median={med:.3f}  stdev={sd:.3f}  CV={cv:.1%}"
    )
    print(
        f"   ({'CONSISTENT -> scale recovered + detections trustworthy' if cv < 0.15 else 'INCONSISTENT -> detections noisy/mis-bound: the solver would EXPOSE this, not hide it'})"
    )
tot = len(anns(real))
bound = sum(1 for d in anns(real) if d.get("refs"))
print(
    f"-> real binding coverage: {bound}/{tot} dims have a primitive ref ({bound / tot:.0%}); "
