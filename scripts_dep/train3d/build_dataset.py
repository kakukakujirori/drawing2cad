...
def main():
    ...
    parser.add_argument(
        "--graph-dir",
        type=str,
        default="experiments/dataset_z2c_val",
    ...
    parser.add_argument(
        "--seed", type=int, default=0, help="RNG seed for --hid-dropout"
    )
    parser.add_argument(
        "--workers",
        type=int,
    ...
    tok = AutoTokenizer.from_pretrained(
        "ADSKAILab/Zero-To-CAD-Qwen3-VL-2B", trust_remote_code=True
    )
    ...
    def process_one(gp):
        ...
        bbox_max = (
            max((abs(x) for x in bbox_vals), default=float("inf"))
            if all(math.isfinite(x) for x in bbox_vals)
            else float("inf")
        )
    ...
    if not records:
        raise RuntimeError(
            "no valid paired records after serialization/quarantine; "
    ...
    for cap in (2048, 3072, 4096, 6144, 8192, 12288, 16384):
        ...
        caps[cap] = {
            "pairs_over": over,
            "pct_over": round(100 * over / len(tot), 1),
            "pairs_kept": len(tot) - over,
        }
    mnt = {
        cap: round(100 * sum(1 for x in tgt if x <= cap) / len(tgt), 1)
        for cap in (768, 1024, 1280, 1536)
    }
    ratios = [
        a["extent_ratio"] if a["extent_ratio"] is not None else float("inf")
    ...
    stats = {
        ...
        "extent_policy": {
            "warn_ratio": args.extent_warn_ratio,
        ...
        "input_tokens": dist(inp),
        "target_tokens": dist(tgt),
        "total_tokens": dist(tot),
        "seq_cap_coverage": caps,
        "max_new_tokens_target_coverage_pct": mnt,
    }
    ...
    for cap, d in caps.items():
        print(
            f"  {cap:5d}: keep {d['pairs_kept']:3d}  over {d['pairs_over']:3d} ({d['pct_over']}%)"
        )
