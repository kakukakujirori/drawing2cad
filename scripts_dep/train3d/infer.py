...
from train_sft import (
    iter_batched_generate,
)  # batched decode (infer.py; in-training eval uses the native Trainer loop)
from serialize import (
    serialize_3d,
    CANON_QUANT,
    coordinate_tokenize_text,
    coordinate_quant_from_vocab,
)
...
def _tok_source(ckpt: str) -> str:
    ...
    if os.path.isdir(ckpt) and os.path.exists(adapter_cfg):
        ...
        if any(
            os.path.exists(os.path.join(ckpt, name))
...
def _export_one(code, out_step, q):
    ...
    try:
        ...
        out["error_msg"] = msg[
            :300
        ]  # persisted per-row + surfaced on stderr (was dropped)
...
def collect_inputs(path: str) -> list[str]:
    ...
    files = glob.glob(os.path.join(path, "*.graph.json")) or glob.glob(
...
def main():
    ...
    parser.add_argument(
        "--ckpt", required=True, help="LoRA adapter dir / full ckpt dir / HF id"
    )
    parser.add_argument(
        "--input", required=True, help="one AMVDG JSON, or a dir of *.graph.json/*.json"
    )
    parser.add_argument(
        "--out", required=True, help="output dir for {stem}.py + {stem}.step"
    )
    parser.add_argument(
        "--max-new-tokens", type=int, default=train_sft.DEFAULT_MAX_NEW_TOKENS
    )
    parser.add_argument(
        "--prompt",
        default=train_sft.PROMPT,
        help='instruction prepended to the graph text (--prompt "" to drop it)',
    )
    parser.add_argument(
        "--timeout", type=float, default=30.0, help="per-sample exec timeout (s)"
    )
    parser.add_argument(
        "--batch-size", type=int, default=16, help="max prompts per generate() call"
    )
    parser.add_argument(
        "--max-batch-tokens",
        type=int,
    ...
    use_coord_tokens = (
        coord_quant is not None
        if args.coord_tokens == "auto"
        else args.coord_tokens == "on"
    )
    if use_coord_tokens and coord_quant is None:
        raise ValueError(
            "--coord-tokens on requires a checkpoint tokenizer with coordinate tokens"
        )
    ...
    print(
        f"loaded tokenizer for {args.ckpt} | {len(inputs)} inputs -> {args.out}",
        file=sys.stderr,
    )
    ...
    for path in inputs:
        ...
        ids = train_sft.chat_ids(
            tok, [{"role": "user", "content": user}], True
        )  # list[int]
        prompts.append((stem, ids))
    prompts.sort(
        key=lambda x: len(x[1])
    )  # length-sorted → minimal padding within a batch
    ...
    cap1 = (
        _tokens_for_gb((_free_gb(0) - MODEL_GB) * MEM_SAFE)
    ...
    if oversize and not can_shard:
        print(
            f"[infer] {len(oversize)} prompt(s) exceed one GPU's ~{cap1} tok capacity and "
            f"sharding is off/unavailable ({n_gpu} GPU, --shard {args.shard}); they will be "
            f"attempted and may OOM-fail.",
            file=sys.stderr,
        )
    ...
    def _run_phase(subset, device_map, label: str):
        ...
        budget = args.max_batch_tokens or _tokens_for_gb(
            sum(_free_gb(d) for d in devs) * MEM_SAFE
        )
        print(
            f"[infer] phase {label}: {len(subset)} prompt(s) | GPU {devs} | "
            f"batch budget ~{budget} tok",
            file=sys.stderr,
        )
        n = 0
        for stem, code in iter_batched_generate(
            model, mtok, subset, args.max_new_tokens, args.batch_size, budget, dev
        ):
            ...
            if n % args.batch_size == 0 or n == len(subset):
                print(
                    f"  [gen {label} {n}/{len(subset)}] {time.time() - t0:.0f}s",
                    file=sys.stderr,
                )
        del model, mtok
        gc.collect()
    ...
    exec_tasks = [
        (stem, (code, os.path.join(args.out, f"{stem}.step")))
    ...
    for stem, res in imap_isolated(exec_tasks, _export_one, args.timeout, workers):
        exec_res[stem] = (
            res
            if res is not None
            else {"exec_ok": False, "step_written": False, "error_type": "timeout"}
        )
        et = exec_res[stem].get("error_type")
        if (
            et not in ("ok", None) and et not in err_sample
        ):  # keep 1 msg+trace per failure class
            err_sample[et] = (
                stem,
                exec_res[stem].get("error_msg", ""),
                exec_res[stem].get("trace", ""),
            )
        done += 1
        if done % 20 == 0:
            print(
                f"  [exec {done}/{len(exec_tasks)}] {time.time() - t0:.0f}s",
                file=sys.stderr,
            )
    ...
    for path in inputs:
        ...
        res = exec_res.get(
            stem,
            {"exec_ok": False, "step_written": False, "error_type": "crash_no_result"},
        )
        n_exec_ok += res["exec_ok"]
        n_step += res["step_written"]
        err_hist[res["error_type"]] = err_hist.get(res["error_type"], 0) + 1
        row = {
            "id": stem,
            "gen": True,
            "exec_ok": res["exec_ok"],
            "step_written": res["step_written"],
    ...
    summary = {
        "n": n,
        "gen_n": sum(r["gen"] for r in rows),
        "exec_ok": n_exec_ok,
        "exec_ok_rate": round(n_exec_ok / n, 4) if n else 0,
        "step_written": n_step,
        "step_rate": round(n_step / n, 4) if n else 0,
    ...
    if n_gen and n_exec_ok == 0:
        ...
        env_like = top_et in ("ImportError", "ModuleNotFoundError", "OSError") or any(
            k in (top_msg or "").lower()
            for k in ("import", "undefined symbol", "cannot open shared")
        )
        why = (
            "ENVIRONMENT fault (e.g. CadQuery/OCP import) — NOT the model"
            if env_like
            else "every exec failed — check the generated code / geometry"
        )
        print("\n" + "=" * 72, file=sys.stderr)
        print(
            f"[infer] ALL {n_gen} exec attempts FAILED, 0 STEP written -> {why}.",
            file=sys.stderr,
        )
        print(
            f"[infer] dominant error: {top_et} ({err_hist[top_et]}/{n_gen})",
            file=sys.stderr,
        )
        if top_msg:
