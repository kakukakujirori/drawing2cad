...
def pred_prims(pred):
    ...
    for v in pred.get("views", []):
        for p in v.get("primitives", []):
            q = dict(p)
            q.setdefault("line_role", "visible")
            q["view"] = v.get("name", "?")
            out.append(q)
    return out
...
def _vis(p):
    return p.get("line_role", p.get("visibility", "visible"))


def _dkind(d):
    return d.get("kind", d.get("type"))  # annotation kind (v0.2 'kind' / v0 'type')


def _anns(g):
    return g.get("annotations", g.get("dimensions", []))
...
def match_curves(gt, pr, vis=None):
    ...
    for g in G:
        ...
        for i, p in enumerate(P):
            if i in used:
                continue
            if d(g["center"], p["center"]) < C_TOL and abs(gr - p.get("r_px", 0)) < max(
                R_TOL, 0.25 * gr
            ):
        ...
        if best is not None:
            used.add(best)
            tp += 1
            pairs.append((g, P[best]))
            if P[best].get("type") == g.get("type"):
                typ += 1
    ...
    for g in G:
        for i, p in enumerate(P):
            if i in used:
                continue
            a = (
                d(g["p1"], p.get("p1", [0, 0])) < L_TOL
                and d(g["p2"], p.get("p2", [0, 0])) < L_TOL
            )
            b = (
                d(g["p1"], p.get("p2", [0, 0])) < L_TOL
                and d(g["p2"], p.get("p1", [0, 0])) < L_TOL
            )
            if a or b:
                used.add(i)
                tp += 1
    ...
    for g in G:
        ...
        for i, p in enumerate(P):
            ...
            if "value" in p and val_match(g["value"], p.get("value", 1e9)):
                if best is None or (_dkind(p) == _dkind(g)):
                    best = i
        if best is not None:
            used.add(best)
            pairs.append((g, P[best]))
...
def main():
    ...
    dia_mae = (
        round(
            sum(abs(g["value"] - p["value"]) for g, p in dia_pairs) / len(dia_pairs), 3
        )
        if dia_pairs
        else None
    )
    ...
    link_tot = 0
    for g, p in dpairs:
        if _dkind(g) not in ("diameter", "radius"):
            continue
        link_tot += 1
        mapped = {pid2gid.get(r) for r in (p.get("refs") or [])}
        if set(g.get("refs", [])) & mapped:
            link_ok += 1
    ...
    m = {
        ...
        "primitives": {
            "curve": {
                "visible": prf(ctp_v, cng_v, cnp_v),
                "all": prf(ctp_a, cng_a, cnp_a),
                "type_agree_rate": type_agree,
                "_note": "circle+arc pooled by center+radius",
            },
            "line": {
                "visible": prf(ltp_v, lng_v, lnp_v),
                "hidden": prf(ltp_h, lng_h, lnp_h),
