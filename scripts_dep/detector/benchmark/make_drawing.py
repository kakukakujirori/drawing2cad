...
def dashed(d, p1, p2, w, dash=10, gap=7):
    x1, y1 = p1; x2, y2 = p2
    L = math.hypot(x2-x1, y2-y1); n = int(L // (dash+gap))
    ux, uy = (x2-x1)/L, (y2-y1)/L
    for i in range(n+1):
        s = i*(dash+gap); e = min(s+dash, L)
        d.line([(x1+ux*s, y1+uy*s), (x1+ux*e, y1+uy*e)], fill=0, width=w)
...
def build():
    ...
    for i in range(4):
        ...
        d.line([a, b], fill=0, width=OUT_W); gt_lines.append([*a, *b])
    # a step edge inside (real internal line)
    d.line([(220, 470), (760, 470)], fill=0, width=OUT_W); gt_lines.append([220, 470, 760, 470])
    # central bore + concentric ring
    cx, cy = 490, 360
    for r in (120, 95):
        d.ellipse([cx-r, cy-r, cx+r, cy+r], outline=0, width=OUT_W); gt_circles.append([cx, cy, r])
    # 4 bolt holes
    for (hx, hy) in [(330, 320), (650, 320), (330, 400), (650, 400)]:
        r = 26
        d.ellipse([hx-r, hy-r, hx+r, hy+r], outline=0, width=OUT_W); gt_circles.append([hx, hy, r])
    ...
    arrow(d, 220, yd, math.pi); arrow(d, 760, yd, 0)
    ...
    arrow(d, xd, 250, -math.pi/2); arrow(d, xd, 640, math.pi/2)
    d.text((xd-40, 430), "390", fill=0, font=font(22))
    # diameter callout on the bore (leader line + text) -> clutter line through a circle
    lx, ly = cx+120*math.cos(math.radians(40)), cy+120*math.sin(math.radians(40))
...
def scan_aug(img, seed=0):
    ...
    im = img.rotate(rng.uniform(-1.5, 1.5), resample=Image.BICUBIC, fillcolor=245, center=(W/2, H/2))
    ...
    m = rng.random(a.shape) < 0.0006; a[m] = rng.integers(20, 90, m.sum())  # speckle
    f = rng.uniform(0.6, 0.8); h, w = a.shape          # resolution loss
    small = Image.fromarray(a.clip(0, 255).astype(np.uint8)).resize((int(w*f), int(h*f)), Image.BILINEAR)
...
def main():
    ...
    print(f"GT: {len(gt['lines'])} real lines, {len(gt['circles'])} real circles "
