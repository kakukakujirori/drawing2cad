...
for c in cnts:
    ...
    cover = (
        len(np.unique((ang * 18 / np.pi).astype(int))) / 36.0
    )  # fraction of 36 angular bins hit
...
for cx, cy, r in cands:
    groups[(round(cx / 12), round(cy / 12))].append((cx, cy, r))
circles = []
for g in groups.values():
    g.sort(key=lambda t: t[2])
    kept = []
    for cx, cy, r in g:
        if not kept or abs(r - kept[-1][2]) / r > 0.06:
            kept.append((cx, cy, r))
    circles += kept
...
def matched(s):
    return sum(1 for r in det if min(abs(r - s * e) / (s * e) for e in exp_r) < TOL)
...
print(
    f"NULL (arbitrary scales): mean {null_mean:.1f}/{len(det)} ({null_mean / max(len(det), 1):.0%}), max {max(nulls)}"
)
signal = nm - null_mean
print(f"signal = best - null_mean = {signal:.1f}")
if det:
    print(
        f"under best s: ⌀200->{s * 100:.0f}px ⌀188->{s * 94:.0f}px ⌀12->{s * 6:.0f}px (max detected r={max(det):.0f})"
    )
    big = [r for r in det if r > s * 40]  # bores bigger than ⌀80
    print(
        f"large circles detected (r> {s * 40:.0f}px ~ ⌀80+): {[round(r) for r in big]}"
    )
verdict = (
    "NON-VACUOUS — classical contour-fit gives a real scale signal"
    if signal >= 4 and len(det) < 150
    else "still vacuous/over-detected — needs trained detector"
    if len(det) >= 150
