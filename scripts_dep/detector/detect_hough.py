...
print(
    "   ink px stroke-width percentiles [10/25/50/75/90/99]:",
    [round(float(q), 1) for q in qs],
)
thr = float(np.percentile(sw, 75))
thick_frac = float((sw > thr).mean())
print(f"   ink coverage: {ink.mean() * 100:.1f}% of page is ink")
print(
    f"   -> thick layer (>p75={thr:.1f}px) = {thick_frac * 100:.0f}% of ink; thin layer = the rest"
)
print(
    "   => a measurable thick/thin split EXISTS (ISO 128 weights survive in the raster)"
)

# thick-ink mask (outline candidates): keep ink whose local stroke width is large
thick = np.zeros_like(bw)
thick[ink & (2 * dt > thr)] = 255
...
dia_mm = sorted(
    {
        d["value"]
        for d in (real.get("annotations") or real.get("dimensions", []))
        if d.get("kind", d.get("type")) == "diameter"
    }
)
...
for s in cands:
    ...
    for i, r in enumerate(det):
        if min(abs(r - s * e) / (s * e) for e in exp_r) < TOL:
            matched.add(i)
...
if best:
    ...
    print(
        f"   best px_per_mm = {s:.3f}  -> matches {nm}/{len(det)} detected circles within {int(TOL * 100)}%"
    )
    # show what the big dims map to under this scale
    print(
        f"   under s={s:.3f}: ⌀200 -> {s * 100:.0f}px, ⌀50 -> {s * 25:.0f}px, ⌀38 -> {s * 19:.0f}px (should be big>small)"
    )

    # NULL TEST: do arbitrary WRONG scales match just as many? (if so, the test is vacuous)
    def matched(ss):
        return sum(
            1 for r in det if min(abs(r - ss * e) / (ss * e) for e in exp_r) < TOL
        )

    nulls = [matched(ss) for ss in np.linspace(3, 12, 40)]
    null_mean = sum(nulls) / len(nulls)
    print(
        f"   NULL TEST: arbitrary wrong scales match mean {null_mean:.0f}/{len(det)} ({null_mean / len(det):.0%}); best={nm} ({nm / len(det):.0%})"
    )
    print(
        f"   maxRadius cap={320}px but ⌀200 would be ~{s * 100:.0f}px -> the big bores are NEVER detectable"
    )
    verdict = (
        "VACUOUS — naive Hough OVER-DETECTS (%d circles vs ~30 real); scale-consistency is meaningless. "
        "A TRAINED clutter-rejecting detector is required (classical CV insufficient, as the literature warns)."
        % len(det)
    )
    print(f"   => {verdict}")
