...
def stage1(
    step_dir: str, out_dir: str, width: int, n: int, workers: int = 0, timeout: int = 60
):
    env = dict(os.environ)
    ...
    cmd = [
        sys.executable,
        os.path.join(root, "scripts", "renderer", "render_dataset.py"),
        "--step-dir",
        step_dir,
        "--out-dir",
        out_dir,
    ...
    print(
        "[stage1] render_dataset: OK=%d SKIP=%d rc=%d" % (nok, nskip, r.returncode),
        flush=True,
    )
    if r.returncode != 0 or (nok == 0 and nskip == 0):
        print(
            "[stage1] WARNING: renderer produced nothing — check %s" % logp, flush=True
        )
...
def _stage2_worker(gp, out_dir, width, do_scan):
    ...
    try:
        ...
        return (
            "FAIL",
            name,
            "corrupt graph.json: %s -- re-render this part via "
...
def stage2(out_dir: str, width: int, do_scan: bool, workers: int = 0):
    ...
    with ProcessPoolExecutor(max_workers=workers) as ex:
        ...
        for res in tqdm(jobs, total=len(graphs), desc="stage2"):
            ...
            if kind == "FAIL":
                print("  raster FAIL", name, payload)
                continue
...
if __name__ == "__main__":
    ...
    if os.path.isdir(args.out_dir) and not args.resume:
        raise FileExistsError(
            f"{args.out_dir=} already exists. Use --resume or remove it first."
        )
    os.makedirs(args.out_dir, exist_ok=True)
    if not args.skip_stage1:
        stage1(
            args.step_dir, args.out_dir, args.width, args.n, args.workers, args.timeout
        )
