...
def hdim(x1, x2, y_geom, y_dim, text):
    ...
    for xx in (x1, x2):
        s.append(
            '<line class="ext" x1="%.2f" y1="%.2f" x2="%.2f" y2="%.2f"/>'
            % (xx, y_start, xx, y_end)
        )
    lo, hi = sorted((x1, x2))
    s.append(
        '<line class="dim" x1="%.2f" y1="%.2f" x2="%.2f" y2="%.2f"/>'
        % (lo, y_dim, hi, y_dim)
    )
    ...
    s.append(
        '<text class="dimtxt" x="%.2f" y="%.2f" text-anchor="middle">%s</text>'
        % (tx, ty, text)
    )
    return "\n".join(s), (tx, ty)
...
def vdim(y1, y2, x_geom, x_dim, text):
    ...
    s.append(
        '<text class="dimtxt" x="%.2f" y="%.2f" text-anchor="middle" '
...
def diadim(cx, cy, r, ang_deg, text):
    ...
    s = [
        '<line class="dim" x1="%.2f" y1="%.2f" x2="%.2f" y2="%.2f"/>' % (x1, y1, lx, ly)
    ]
    s.append(_arrow(x1, y1, a + math.pi))
    s.append(_arrow(x2, y2, a))
    anchor = "start" if math.cos(a) >= 0 else "end"
    tx = lx + (1.5 if anchor == "start" else -1.5)
    ty = ly - 1.2
    s.append(
        '<text class="dimtxt" x="%.2f" y="%.2f" text-anchor="%s">%s</text>'
        % (tx, ty, anchor, text)
    )
    return "\n".join(s), (tx, ty)
...
def extract_cylinders(shape):
    ...
    if rejected:
        log.warning(
            "dimensioner: rejected %d out-of-envelope cylinder surface(s)", rejected
        )
    seen = {}
    uniq = []
    for fe in feats:
        ...
        seen[key] = len(uniq)
        fe = dict(fe)
        fe["id"] = "cyl%d" % len(uniq)
        fe["face_ids"] = [fe.pop("face_id")]
        uniq.append(fe)
    return uniq
...
class LegacyDimensioner:
    ...
    def _find_circle_prim(self, view_name, center_cu, center_cv, r, tolpx=4.0):
        ...
        for p in self.view_by_name[view_name]["primitives"]:
            if p["type"] not in ("circle", "arc"):
                continue
            d = math.hypot(p["center"][0] - tx, p["center"][1] - ty) + abs(
                p["r_px"] - rpx
            )
            if d < bestd:
                bestd = d
    ...
    def _record_centerlines(self, vname, sx, sy, L, cyl):
        ...
        origins = [
            {"dim": 2, "id": fid, "role": "axis"} for fid in cyl.get("face_ids", [])
        ]
        for x1, y1, x2, y2 in ((sx - L, sy, sx + L, sy), (sx, sy - L, sx, sy + L)):
            ...
            prims.append(
                {
                    "id": "%s%d" % (pfx, len(prims)),
                    "type": "line",
                    "line_role": "center",
                    "feature_tag": "hole_or_round",
                    "feature_id": cyl["id"],
                    "state": "known",
                    "coords_source": "gt",
                    "p1": p1,
                    "p2": p2,
                    "bbox_px": [
                        round(min(p1[0], p2[0]), 1),
                        round(min(p1[1], p2[1]), 1),
                        round(max(p1[0], p2[0]), 1),
                        round(max(p1[1], p2[1]), 1),
                    ],
    ...
    def _centerline_in_view(self, vname, sx, sy, L):
        ...
        return (
            ...
            and math.isfinite(L)
            and L > 0
            and sx - L >= xmin - margin
            and sx + L <= xmax + margin
            and sy - L >= ymin - margin
            and sy + L <= ymax + margin
        )
        ...
        self.dims_gt.append(
            {
                ...
                "text_px": [
                    round(anchor_mm[0] * self.PXMM, 2),
                    round(anchor_mm[1] * self.PXMM, 2),
                ],
    ...
    def annotate(self):
        ...
        _axis_view = {
            "Y": ("front", front, lambda c: (c[0], c[2])),
        ...
        for c in sorted(cyls, key=lambda c: -c["r"]):
            ...
            if not self._centerline_in_view(vname, sx, sy, line_len):
                ...
                log.warning(
                    "dimensioner: rejected out-of-view centerline for %s in %s",
                    c["id"],
                    vname,
                )
                continue
        ...
        for c in cyls:
            ...
            if not members:
                ...
                if probe:
                    rid = self._find_circle_prim(
                        probe[0], probe[1][0], probe[1][1], c["r"]
                    )
                    if rid:
                        members = [
                            {
